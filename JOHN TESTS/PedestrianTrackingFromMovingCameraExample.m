...
% Original dimensions = [40 95 400 140]
...
option.ageThresh            = 2; %originally 8  % A threshold to determine the minimum length required for a track being true positive.
...
stopFrame = 1629; % original: 1629; stop on an interesting frame with several pedestrians
...
% <matlab:doc('imageLabeler'); imageLabeler>
...
%                         classification score at the corresponding frame.
    ...
    function [centroids, bboxes, scores] = detectPeople()
        ...
        % Calculate resize ratio based on video size
        % If the video is too small, make bigger
        if(vidHeight < 300 || vidWidth < 320)
            resizeRatio = 2;
        else
            resizeRatio = 1;
        end
        
        % If the video is too big, make smaller
        if(vidHeight > 600 || vidWidth > 600)
            resizeRatio = 0.75;
        else
            resizeRatio = 1;
        end
        
        %resizeRatio = 1; % was originally 1.5
...
% to eliminate noisy detections, such as size, location, or appearance.
    ...
    function createNewTracks()
        ...
        for i = 1:size(unassignedBboxes, 1)            
            ...
            newTrack = struct(... 
                'id', nextId, ...
                'color', [255,255,255], ... %255*rand(1,3)
                'bboxes', bbox, ...
                'scores', score, ...
...
% detections and tracks.
    ...
    function displayTrackingResults()
        ...
        if ~isempty(tracks),
            ...
            %opacity = min(0.5,max(0.1,avgConfidence/3)); <- old code
            %opacity = 0; <- don't need this, was used with filled
            %rectangle portion
            ...
            for i = 1:length(tracks)
                if ~noDispInds(i)
                    ...
                    frame = insertObjectAnnotation(frame, ...
                                            'rectangle', bb, ... % draws rectangular border around object
                                            'A PEOPLE PERSON THANG', ... % the text above the bounding box
                                            'Color', [255,255,255], ... % color of bounding box
                                            'LineWidth', 3); % line thickness of bounding box
                end
            end
        end
        
        %Adds a border frame to our search area
